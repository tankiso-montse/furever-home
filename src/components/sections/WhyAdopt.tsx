...
function WhyAdopt() {
  ...
  return (
    <motion.div className='w-full h-screen bg-white md:p-12 lg:p-20'>
      <div className='h-full flex flex-col items-center justify-around lg:justify-between xl:px-14 gap-12 md:gap-0'>
        <motion.h1 {...fadeIn} transition={{ duration: 0.6, ease: 'easeInOut' }} className='text-secondary text-6xl tracking-tighter font-instrument-sans font-semibold'>WHY ADOPT</motion.h1>
        <motion.div {...fadeIn} transition={{ duration: 0.8, ease: 'easeInOut'}} className='grid grid-rows-2 lg:grid-rows-1 lg:grid-cols-2 lg:w-full xl:w-11/12 min-h-96 lg:min-h-80 rounded-4xl overflow-hidden bg-green-500'>
        ...
        <div className='w-11/12 h-32 flex flex-col gap-12 items-center'>
          <motion.div {...fadeIn} transition={{ duration: 0.6, ease: 'easeInOut'}} className='flex gap-8'>
            {adoptionBenefits.map((adoptionBenefit, index) => (
              <Button
                key={index}
                onClick={() => setActiveButton(adoptionBenefit.title)}
                extraClasses={`${activeButton === adoptionBenefit.title ? 'bg-secondary text-white transition-all duration-300' : ''}`}
              >{adoptionBenefit.title}</Button>
            ))}
          </motion.div>
          {activeButton === adoptionBenefits.find(benefit => benefit.title === activeButton)?.title && (
            <motion.p key={activeButton} {...fadeIn} transition={{ duration: 0.8, ease: 'easeInOut'}} className='max-w-3xl text-center font-instrument-sans text-lg text-gray-700'>{adoptionBenefits.find(benefit => benefit.title === activeButton)?.description}</motion.p>
          )}
        </div>
      </div>
    </motion.div>
